#ifndef DGMETHODZERO_H
#define DGMETHODZERO_H

// DGMethodZero.h describes an abstract interface and functions for a DG method with zero Dirichlet boundaries
#include <functional>
#include <set>
#include "../Point.h"
#include "../Parameter.h"
...
namespace corenc
{
    ...
    namespace method
    {
        ...
        public:
            ...
            DGMethodZero(const std::shared_ptr<Grid>& grid) :m_Grid{ grid->Clone() } {}
            DGMethodZero(Grid* grid) :m_Grid{ grid->Clone() } {}
        ...
        template<class Problem, class Grid, class Matrix>
        void DGMethodZero<Problem, Grid, Matrix>::Discretization()
        {
            GeneratePortrait();
            AssemblGlobal();
            AssembleInter();
        ...
        template<class Problem, class Grid, class Matrix>
        void DGMethodZero<Problem, Grid, Matrix>::GeneratePortrait()
        {
            ...
            for (k = 0; k < m_N; ++k)
            {
                ...
                for (i = 0; i < order; ++i)
                {
                    if (m_Grid->interpolate(elem->GetNode(i)) > -1)
                    {
                        m_inums[k][i] = size;
                        ++size;
                    }
            ...
            for (k = 0; k < sz; ++k)
            {
                ...
                if (ne != -1)
                {
                    ...
                    for (i = 0; i < order; ++i)
                    {
                        int inode = m_Grid->interpolate(elemk->GetNode(i));
                        if (inode > -1)
                        {
                            //sizej = sizei + 1;
                            for (j = i + 1; j < order; ++j)
                            {
                                int jnode = m_Grid->interpolate(elemk->GetNode(j));
                                if (jnode > - 1)
                                {
                        ...
                        if (inode > -1)
                        {
                            ...
                            for (j = 0; j < order; ++j)
                            {
                                ...
                                if (jnode > - 1)
                                {
                                    ...
                                    //temp[m_nums[ne] + sizej].insert(m_nums[nk] + sizei);
                                    ++sizej;
                                    std::cout << "k";
                                }
                            }
        ...
        template<class Problem, class Grid, class Matrix>
        void DGMethodZero<Problem, Grid, Matrix>::AssemblGlobal()
        {
            int l;
        ...
        template<class Problem, class Grid, class Matrix>
        const int DGMethodZero<Problem, Grid, Matrix>::AssembleIDUDVMatrix(const int l)
        {
            int i, j, k, nodes;
            double mij;
            ...
            for (i = 0; i < (int)dofs; ++i)
            {
                ...
                for (j = 0; j < (int)dofs; ++j)
                {
                    ...
                    m_GlobalMatrix->AddElement(m_nums[l] + sizei, m_nums[l] + sizej, mij);
                    ++sizej;
                }
                ++sizei;
            }
        ...
        template<class Problem, class Grid, class Matrix>
        const int DGMethodZero<Problem, Grid, Matrix>::AssembleIDUVMatrix(const int l)
        {
            int i, j, k, nodes;
            double mij;
            const auto& elem{ m_Grid->GetElement(l) };
            ...
            for (i = 0; i < (int)dofs; ++i)
            {
                ...
                for (j = 0; j < (int)dofs; ++j)
                {
                    ...
                    m_GlobalMatrix->AddElement(m_nums[l] + sizei, m_nums[l] + sizej, _mij);
                    ++sizej;
                }
                ++sizei;
            }
        ...
        template<class Problem, class Grid, class Matrix>
        const int DGMethodZero<Problem, Grid, Matrix>::AssembleIUDVMatrix(const int l)
        {
            int i, j, k, nodes;
            double mij;
            const auto& elem{ m_Grid->GetElement(l) };
        ...
        template<class Problem, class Grid, class Matrix>
        const int DGMethodZero<Problem, Grid, Matrix>::AssembleRUVMatrix(const int l)
        {
            int i, j, k, nodes;
            double mij;
            const auto& elem{ m_Grid->GetElement(l) };
            ...
            for (i = 0; i < (int)dofs; ++i)
            {
                ...
                for (j = 0; j < (int)dofs; ++j)
                {
                    ...
                    if (inode > -1 && jnode > -1)
                    {
                        m_RightMatrix->AddElement(m_nums[l] + sizei, m_nums[l] + sizej, mij);
                        ++sizej;
                    }
                }
                ++sizei;
        ...
        template<class Problem, class Grid, class Matrix>
        const int DGMethodZero<Problem, Grid, Matrix>::AssembleSUPGMatrix(const int l)
        {
            int i, j, k, nodes;
            double mij;
            const auto& elem{ m_Grid->GetElement(l) };
        ...
        template<class Problem, class Grid, class Matrix>
        const int DGMethodZero<Problem, Grid, Matrix>::AssembleInter()
        {
            for (int l = 0; l < m_Ns; ++l)
            {
                ...
                for (int i = 0; i < dofsk; ++i)
                {
                    ...
                    for (int j = 0; j < dofsk; ++j)
                    {
                        ...
                        auto Tkk = [&](const Mesh::Point& p)
                        {
                            ...
                            return 0.5 * kappa * (val2 - val1);
                        };
                        auto mj = bound->Integrate(Tkk, points);
                        m_GlobalMatrix->AddElement(m_nums[nk] + m_inums[nk][i], m_nums[nk] + m_inums[nk][j], mj);
                    }
                }
                ...
                for (int i = 0; i < dofsk; ++i)
                {
                    ...
                    for (int j = 0; j < dofsk; ++j)
                    {
                        ...
                        auto mj = bound->Integrate(Tkk, points);
                        m_GlobalMatrix->AddElement(m_nums[nk] + m_inums[nk][i], m_nums[ne] + m_inums[ne][j], mj);
                    }
                }
            }
            return 0;
        ...
        template<class Problem, class Grid, class Matrix>
        const int DGMethodZero<Problem, Grid, Matrix>::AssembleLocalMatrix(const int l, const int old)
        {
            int i, j, k, nodes;
            double mij;
            const auto& elem{ m_Grid->GetElement(l) };
        ...
        template<class Problem, class Grid, class Matrix>
        void DGMethodZero<Problem, Grid, Matrix>::MainConditions()
        {
            double mu{ 1e10 };
        ...
        template<class Problem, class Grid, class Matrix>
        void DGMethodZero<Problem, Grid, Matrix>::SecondConditions()
        {
            double theta = 0;
            int nfem;
            Mesh::Point temp[3];
        ...
        template<class Problem, class Grid, class Matrix>
        void DGMethodZero<Problem, Grid, Matrix>::StefanConditions()
        {
            double dest{ 0. }, lat{ 0 };
            int nfem;
            Mesh::Point temp[3];
        ...
        template<class Problem, class Grid, class Matrix>
        void DGMethodZero<Problem, Grid, Matrix>::ThirdConditions()
        {
        ...
        template<class Problem, class Grid, class Matrix>
        Matrix* DGMethodZero<Problem, Grid, Matrix>::GetGlobalMatrix() const
        {
            return m_GlobalMatrix;
        }
        template<class Problem, class Grid, class Matrix>
        const double DGMethodZero<Problem, Grid, Matrix>::GetValue(const Mesh::Point& p) const
        {
            if (!m_solution.size())
                return -1;
            double val = 0;
        ...
        template<class Problem, class Grid, class Matrix>
        const double DGMethodZero<Problem, Grid, Matrix>::GetValue(const Mesh::Point& p, const std::vector<double>& vec) const
        {
            if (!vec.size())
                return -1;
            double val{ 0 };
        ...
        template<class Problem, class Grid, class Matrix>
        const double DGMethodZero<Problem, Grid, Matrix>::GetValue(const Mesh::Point& p, const std::vector<double>& vec, const int num) const
        {
            if (!vec.size() || num < 0)
                return -1;
            double val{ 0 };
        ...
        //template<class Problem, class Grid, class Matrix>
        //const Mesh::Point DGMethodZero<Problem, Grid, Matrix>::GetGradValue(const Mesh::Point& p, const std::vector<double>& vec) const
        //{
        //	Mesh::Point val{ 0, 0 };
        //	int nfem{ -1 };
        //	nfem = m_Grid->FindElement(p);
        ...
        //}
        template<class Problem, class Grid, class Matrix>
        const double DGMethodZero<Problem, Grid, Matrix>::GetEffective(const std::vector<double>& vec) const
        {
            double sum = 0;
            //std::vector<int> dofs;
        ...
        //template<class Problem, class Grid, class Matrix>
        //const Mesh::Point DGMethodZero<Problem, Grid, Matrix>::GetLambdaGrad(const Mesh::Point& p, const std::vector<double>& vec) const
        //{
        ...
        //}
        template<class Problem, class Grid, class Matrix>
        const std::vector<double> DGMethodZero<Problem, Grid, Matrix>::GetRightVector() const
        {
            return *m_rhsvector;
        }
        template<class Problem, class Grid, class Matrix>
        void DGMethodZero<Problem, Grid, Matrix>::OutDatFormat(const Mesh::Point& mn, const Mesh::Point& mx, const std::string& file_name, const std::vector<double>& vec) const
        {
        ...
        template<class Problem, class Grid, class Matrix>
        void DGMethodZero<Problem, Grid, Matrix>::ApplySources()
        {
            int nfem = -1;
        ...
        template<class Problem, class Grid, class Matrix>
        void DGMethodZero<Problem, Grid, Matrix>::Rediscretization(const std::shared_ptr<Grid>& grid)
        {
        ...
        template<class Problem, class Grid, class Matrix>
        void DGMethodZero<Problem, Grid, Matrix>::Rediscretization()
        {
            m_time += m_step;
            m_GlobalMatrix->NullMatrix();
            for (unsigned int i = 0; i < m_rhsvector->size(); ++i)
        ...
        template<class Problem, class Grid, class Matrix>
        void DGMethodZero<Problem, Grid, Matrix>::GetSolution(std::vector<double>& vec)
        {
            int size = vec.size();
            //Translation(vec);
        ...
        template<class Problem, class Grid, class Matrix>
        const double DGMethodZero<Problem, Grid, Matrix>::GetSolution(const Grid& g, const std::vector<double> &weights, const Mesh::Point& p)
        {
            double sum{ 0 };
        ...
        template<class Problem, class Grid, class Matrix>
        const double DGMethodZero<Problem, Grid, Matrix>::GetSolution(const Grid& g, const std::vector<double> &weights, const Mesh::Point& p, const int nfem)
        {
            double sum{ 0 };
            //if (nfem < 0)
            //    return 0.;
        ...
        template<class Problem, class Grid, class Matrix>
        const Mesh::Point DGMethodZero<Problem, Grid, Matrix>::GetGradSolution(const Grid& g, const std::vector<double> &weights, const Mesh::Point& p)
        {
        ...
        template<class Problem, class Grid, class Matrix>
        const Mesh::Point DGMethodZero<Problem, Grid, Matrix>::GetGradSolution(const Grid& g, const std::vector<double> &weights, const Mesh::Point& p, const int nfem)
        {
            Mesh::Point sum{ 0, 0, 0 };
            auto elem{ g.GetElement(nfem) };
            auto dofs{ elem->GetDoFs() };
        ...
        template<class Problem, class Grid, class Matrix>
        void DGMethodZero<Problem, Grid, Matrix>::LoadSolution(const std::vector<double>& vec)
        {
        ...
        template<class Problem, class Grid, class Matrix>
        void DGMethodZero<Problem, Grid, Matrix>::OutMeshFormat(const std::string& file_name, const std::vector<double>& vec)
        {
        ...
        template<class Problem, class Grid, class Matrix>
        void DGMethodZero<Problem, Grid, Matrix>::OutMeshTimeFormat(const std::string& file_name, const std::vector<double>& vec)
        {
            const int size{ (int)m_Grid->GetNodes().size() };
            const int number{ (int)m_Grid->GetElements().size() };
            //const int size{ number * 4 };
        ...
        template<class Problem, class Grid, class Matrix>
        void DGMethodZero<Problem, Grid, Matrix>::ProjectSolution(std::vector<double>& sol, std::function<const double(const Mesh::Point&, const std::vector<double>&, const int)> GetVal, std::vector<double>& vec)
        {
            for (int i = 0; i < m_Grid->GetElements().size(); ++i)
            {
                auto elem = m_Grid->GetElements()[i];
        ...
        template<class Problem, class Grid, class Matrix>
        void DGMethodZero<Problem, Grid, Matrix>::ProjectSolution(std::vector<double>& sol, std::function<const double(const Mesh::Point&, const std::vector<double>&)> GetVal, std::vector<double>& vec, const int)
        {
            for (int i = 0; i < m_Grid->GetElements().size(); ++i)
            {
                auto elem = m_Grid->GetElements()[i];
        ...
        template<class Problem, class Grid, class Matrix>
        const std::vector<double> DGMethodZero<Problem, Grid, Matrix>::SetSolution(const int sol, const int liq, const double s, const double l, const double m)
        {
            int i;
        ...
        template<class Problem, class Grid, class Matrix>
        DGMethodZero<Problem, Grid, Matrix>::~DGMethodZero()
        {
            delete m_Grid;
        }
    }
...
#endif // !CORENC_METHODS_DGMethodZero_h
