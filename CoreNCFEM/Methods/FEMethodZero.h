// FEMethodZero.h describes an abstract interface and functions for a general finite element method with zero Dirichlet boundaries
#ifndef CORENC_METHODS_FEMethodZeroZero_h
#define CORENC_METHODS_FEMethodZeroZero_h
#include <functional>
#include <set>
#include "../Point.h"
#include "../Parameter.h"
...
namespace corenc
{
    ...
    namespace method
    {
        ...
        public:
            ...
            FEMethodZero(const std::shared_ptr<Grid>& grid) :m_Grid{ grid->Clone() } {}
            FEMethodZero(Grid* grid) :m_Grid{ grid->Clone() } {}
        ...
        template<class Problem, class Grid, class Matrix>
        void FEMethodZero<Problem, Grid, Matrix>::Discretization()
        {
            GeneratePortrait();
            AssemblGlobal();
            //ApplySources();
        ...
        template<class Problem, class Grid, class Matrix>
        void FEMethodZero<Problem, Grid, Matrix>::GeneratePortrait()
        {
            ...
            for (k = 0; k < m_N; ++k)
            {
                ...
                for (i = 0; i < order; ++i)
                    for (j = 0; j < order; ++j)
                    {
                        ...
                        int jnode = m_Grid->interpolate(elem->GetNode(j));
                        int inode = m_Grid->interpolate(elem->GetNode(i));
                        //std::cout << jnode << "\t" << inode << std::endl;
                        //std::cout << "outside" << std::endl;
                        if (jnode > -1 && inode > -1)
                            if (jnode > inode)
                            {
                                temp[jnode].insert(inode);
                            }
                    }
            }
        ...
        template<class Problem, class Grid, class Matrix>
        void FEMethodZero<Problem, Grid, Matrix>::AssemblGlobal()
        {
            int l;
        ...
        template<class Problem, class Grid, class Matrix>
        const int FEMethodZero<Problem, Grid, Matrix>::AssembleIDUDVMatrix(const int l)
        {
            int i, j, k, nodes;
            double mij;
        ...
        template<class Problem, class Grid, class Matrix>
        const int FEMethodZero<Problem, Grid, Matrix>::AssembleIDUVMatrix(const int l)
        {
            int i, j, k, nodes;
            double mij;
            const auto& elem{ m_Grid->GetElement(l) };
        ...
        template<class Problem, class Grid, class Matrix>
        const int FEMethodZero<Problem, Grid, Matrix>::AssembleIUDVMatrix(const int l)
        {
            int i, j, k, nodes;
            double mij;
            const auto& elem{ m_Grid->GetElement(l) };
        ...
        template<class Problem, class Grid, class Matrix>
        const int FEMethodZero<Problem, Grid, Matrix>::AssembleRUVMatrix(const int l)
        {
            int i, j, k, nodes;
            double mij;
            const auto& elem{ m_Grid->GetElement(l) };
            ...
            auto MM = [&](const Mesh::Point& p)
            {
                ...
                //h = fabs(points[1].x - points[0].x);
                h = sqrt(h);
                //h = fabs(m_Grid->GetNode(0).x - m_Grid->GetNode(1).x);
                ...
                for (j = 0; j < (int)dofs; ++j)
                {
                    ...
                    auto M = [&](const Mesh::Point& p)
                    {
                        ...
                        if (Pe >= 1)
                            ...
                            //tau = 0.;
                        //tau = 1e-7;
                        //std::cout << "tau Pe:\t" << Pe << std::endl;
                        //std::cout << "tau vel:\t" << vel << std::endl;
                        //std::cout << "tau h:\t" << h << std::endl;
        ...
        template<class Problem, class Grid, class Matrix>
        const int FEMethodZero<Problem, Grid, Matrix>::AssembleSUPGMatrix(const int l)
        {
            int i, j, k, nodes;
            double mij;
            const auto& elem{ m_Grid->GetElement(l) };
            ...
            for (i = 0; i < (int)dofs; ++i)
            {
                for (j = 0; j < (int)dofs; ++j)
                {
                    ...
                    auto M = [&](const Mesh::Point& p)
                    {
                        ...
                        //return 0.;
                        //tau = 1e-7;
                        //std::cout << "Stau Pe:\t" << Pe << std::endl;
                        //std::cout << "Stau vel:\t" << vel << std::endl;
                        //std::cout << "Stau h:\t" << h << std::endl;
                        //std::cout << "Stau:\t" << tau << std::endl;
                        auto ret = tau * m_problem->get_parameter(Terms::IDUV, l, elem->GetType(), p, 0) * m_problem->get_parameter(Terms::IDUV, l, elem->GetType(), p, 0) *
                                elem->GetGradShapeFunction(i, p) * elem->GetGradShapeFunction(j, p);
        ...
        template<class Problem, class Grid, class Matrix>
        const int FEMethodZero<Problem, Grid, Matrix>::AssembleLocalMatrix(const int l, const int old)
        {
            int i, j, k, nodes;
            double mij;
            const auto& elem{ m_Grid->GetElement(l) };
        ...
        template<class Problem, class Grid, class Matrix>
        void FEMethodZero<Problem, Grid, Matrix>::MainConditions()
        {
            double mu{ 1e10 };
        ...
        template<class Problem, class Grid, class Matrix>
        void FEMethodZero<Problem, Grid, Matrix>::SecondConditions()
        {
            double theta = 0;
            int nfem;
            Mesh::Point temp[3];
        ...
        template<class Problem, class Grid, class Matrix>
        void FEMethodZero<Problem, Grid, Matrix>::StefanConditions()
        {
            double dest{ 0. }, lat{ 0 };
            int nfem;
            Mesh::Point temp[3];
        ...
        template<class Problem, class Grid, class Matrix>
        void FEMethodZero<Problem, Grid, Matrix>::ThirdConditions()
        {
        ...
        template<class Problem, class Grid, class Matrix>
        Matrix* FEMethodZero<Problem, Grid, Matrix>::GetGlobalMatrix() const
        {
            return m_GlobalMatrix;
        }
        template<class Problem, class Grid, class Matrix>
        const double FEMethodZero<Problem, Grid, Matrix>::GetValue(const Mesh::Point& p) const
        {
            if (!m_solution.size())
                return -1;
            double val = 0;
        ...
        template<class Problem, class Grid, class Matrix>
        const double FEMethodZero<Problem, Grid, Matrix>::GetValue(const Mesh::Point& p, const std::vector<double>& vec) const
        {
            if (!vec.size())
                return -1;
            double val{ 0 };
        ...
        template<class Problem, class Grid, class Matrix>
        const double FEMethodZero<Problem, Grid, Matrix>::GetValue(const Mesh::Point& p, const std::vector<double>& vec, const int num) const
        {
            if (!vec.size() || num < 0)
                return -1;
            double val{ 0 };
        ...
        //template<class Problem, class Grid, class Matrix>
        //const Mesh::Point FEMethodZero<Problem, Grid, Matrix>::GetGradValue(const Mesh::Point& p, const std::vector<double>& vec) const
        //{
        //	Mesh::Point val{ 0, 0 };
        //	int nfem{ -1 };
        //	nfem = m_Grid->FindElement(p);
        ...
        //}
        template<class Problem, class Grid, class Matrix>
        const double FEMethodZero<Problem, Grid, Matrix>::GetEffective(const std::vector<double>& vec) const
        {
            double sum = 0;
            //std::vector<int> dofs;
        ...
        //template<class Problem, class Grid, class Matrix>
        //const Mesh::Point FEMethodZero<Problem, Grid, Matrix>::GetLambdaGrad(const Mesh::Point& p, const std::vector<double>& vec) const
        //{
        ...
        //}
        template<class Problem, class Grid, class Matrix>
        const std::vector<double> FEMethodZero<Problem, Grid, Matrix>::GetRightVector() const
        {
            return *m_rhsvector;
        }
        template<class Problem, class Grid, class Matrix>
        void FEMethodZero<Problem, Grid, Matrix>::OutDatFormat(const Mesh::Point& mn, const Mesh::Point& mx, const std::string& file_name, const std::vector<double>& vec) const
        {
        ...
        template<class Problem, class Grid, class Matrix>
        void FEMethodZero<Problem, Grid, Matrix>::ApplySources()
        {
            int nfem = -1;
        ...
        template<class Problem, class Grid, class Matrix>
        void FEMethodZero<Problem, Grid, Matrix>::Rediscretization(const std::shared_ptr<Grid>& grid)
        {
        ...
        template<class Problem, class Grid, class Matrix>
        void FEMethodZero<Problem, Grid, Matrix>::Rediscretization()
        {
            m_time += m_step;
            m_GlobalMatrix->NullMatrix();
            for (unsigned int i = 0; i < m_rhsvector->size(); ++i)
        ...
        template<class Problem, class Grid, class Matrix>
        void FEMethodZero<Problem, Grid, Matrix>::GetSolution(std::vector<double>& vec)
        {
            int size = vec.size();
            //Translation(vec);
        ...
        template<class Problem, class Grid, class Matrix>
        const double FEMethodZero<Problem, Grid, Matrix>::GetSolution(const Grid& g, const std::vector<double> &weights, const Mesh::Point& p)
        {
            double sum{ 0 };
        ...
        template<class Problem, class Grid, class Matrix>
        const double FEMethodZero<Problem, Grid, Matrix>::GetSolution(const Grid& g, const std::vector<double> &weights, const Mesh::Point& p, const int nfem)
        {
            double sum{ 0 };
            //if (nfem < 0)
            //    return 0.;
        ...
        template<class Problem, class Grid, class Matrix>
        const Mesh::Point FEMethodZero<Problem, Grid, Matrix>::GetGradSolution(const Grid& g, const std::vector<double> &weights, const Mesh::Point& p)
        {
        ...
        template<class Problem, class Grid, class Matrix>
        const Mesh::Point FEMethodZero<Problem, Grid, Matrix>::GetGradSolution(const Grid& g, const std::vector<double> &weights, const Mesh::Point& p, const int nfem)
        {
            Mesh::Point sum{ 0, 0, 0 };
            auto elem{ g.GetElement(nfem) };
            auto dofs{ elem->GetDoFs() };
        ...
        template<class Problem, class Grid, class Matrix>
        void FEMethodZero<Problem, Grid, Matrix>::LoadSolution(const std::vector<double>& vec)
        {
        ...
        template<class Problem, class Grid, class Matrix>
        void FEMethodZero<Problem, Grid, Matrix>::OutMeshFormat(const std::string& file_name, const std::vector<double>& vec)
        {
        ...
        template<class Problem, class Grid, class Matrix>
        void FEMethodZero<Problem, Grid, Matrix>::OutMeshTimeFormat(const std::string& file_name, const std::vector<double>& vec)
        {
            const int size{ (int)m_Grid->GetNodes().size() };
            const int number{ (int)m_Grid->GetElements().size() };
            //const int size{ number * 4 };
        ...
        template<class Problem, class Grid, class Matrix>
        void FEMethodZero<Problem, Grid, Matrix>::ProjectSolution(std::vector<double>& sol, std::function<const double(const Mesh::Point&, const std::vector<double>&, const int)> GetVal, std::vector<double>& vec)
        {
            for (int i = 0; i < m_Grid->GetElements().size(); ++i)
            {
                auto elem = m_Grid->GetElements()[i];
        ...
        template<class Problem, class Grid, class Matrix>
        void FEMethodZero<Problem, Grid, Matrix>::ProjectSolution(std::vector<double>& sol, std::function<const double(const Mesh::Point&, const std::vector<double>&)> GetVal, std::vector<double>& vec, const int)
        {
            for (int i = 0; i < m_Grid->GetElements().size(); ++i)
            {
                auto elem = m_Grid->GetElements()[i];
        ...
        template<class Problem, class Grid, class Matrix>
        const std::vector<double> FEMethodZero<Problem, Grid, Matrix>::SetSolution(const int sol, const int liq, const double s, const double l, const double m)
        {
            int i;
        ...
        template<class Problem, class Grid, class Matrix>
        FEMethodZero<Problem, Grid, Matrix>::~FEMethodZero()
        {
            delete m_Grid;
        }
    }
...
#endif // !CORENC_METHODS_FEMethodZeroZero_h
