#ifndef CORENC_MESH_TRIANGLE_H_
#define CORENC_MESH_TRIANGLE_H_
...
namespace corenc
{
	namespace Mesh
	{
		...
		public:
			...
			const double										GetMeasure() const { return fabs(m_det); };
		...
		private:
			...
			Point                                               m_normal;
			double                                              m_det;
			const double                                        m_L(const int, const Point&) const;
			const double										m_xi(const int, const Point&) const;
        ...
        class CTriangleLagrangeBasis : public CShapeFunction<double>
        {
        public:
            ...
            //CSolution*										GetValue(const unsigned int);
            //const CFESolution									GetValue(const int) const;
            const double                                        GetAlpha(const int i, const int j) const { return m_alpha[i][j]; }
            const double										GetMeasure() const { return fabs(m_det); };
            const double										GetWeight(const int, const std::vector<Point>& verts, const std::function<const double(const Point&)>& f) const;
            //const unsigned int									GetOrder() const;
            //const std::function<const DForm<0>*(const Point&)>  GetShapeFunction(const int) const;
