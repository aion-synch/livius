...
const double CRegularMesh::CompSquare(const Point& p1, const Point& p2, const Point& p3) const
{
...
CRegularMesh::~CRegularMesh()
{
	if (m_elems.size() > 0)
	{
		...
		for (auto i = 0; i < sz; ++i)
			delete m_elems[i];
        vector<CElement2D<>*>().swap(m_elems);
	}
	if (m_edges.size() > 0)
	{
	...
	if (m_elemsbase.size() > 0)
	{
        ...
        vector<CElement2D<>*>().swap(m_elemsbase);
	}
...
CRegularMesh::CRegularMesh(const string& file_name)
{
	//cout << file_name << endl;
	...
	CFiniteElement<CRectangle, CRectangleBasis, int> ele{};
	...
	try
	{
		...
		for (int i = 0; i < nums; ++i)
		{
			...
			case 2:
			{
				...
				//points[1] = m_points[nods[1]];
				//points[2] = m_points[nods[2]];
				CFiniteElement<CRectangle, CRectangleBasis> tr{ CFiniteElement<CRectangle, CRectangleBasis>{&nods[0], points, 3, tp} };
                ...
                m_elems.push_back(new CFiniteElement2D<CRectangle, CRectangleHBasis>{ &nods[0], points, 3, tp });
	...
	for (int i = 0; i < sz_el; ++i)
	{
		...
		points[1] = m_points[nods[1]];
		points[2] = m_points[nods[2]];
        m_elemsbase[i] = new CFiniteElement2D<CRectangle, CRectangleHBasis>{ &nods[0], points, 3, m_elems[i]->GetType() };
	}
...
CRegularMesh::CRegularMesh(const CRegularMesh &tr)
{
...
CRegularMesh::CRegularMesh(const double x1, const double y1, const double x2, const double y2, const int nx, const int ny)
{
	...
	for (size_t i = 0; i < ny; ++i)
	{
		for (size_t j = 0; j < nx; ++j)
		{
            ...
            m_elems.push_back(new CFiniteElement2D<CRectangle, CRectangleHBasis>{ nodes, temp, 4, 0 });
		}
	}
...
CRegularMesh::CRegularMesh(const Point& p1, const Point& p2, const int nx, const int ny)
{
    const auto x1 = p1.x;
    const auto y1 = p1.y;
    const auto x2 = p2.x;
...
CRegularMesh::CRegularMesh(const Point& p1, const Point& p2, const int nx, const int ny, const int px, const int py)
{
    /*if ((px == 2) && (py == 1))
    {
        const auto x1 = p1.x;
        ...
        for (size_t i = 0; i < ny; ++i)
        {
            for (size_t j = 0; j < nx; ++j)
            {
                ...
                m_elems.push_back(new CFiniteElement2D<CRectangle, CRectangleBasis2x>{ nodes, temp, 6, 0 });
    ...
    else if ((py == 2) && (px == 1))
    {
        ...
        for (size_t i = 0; i < ny; ++i)
        {
            for (size_t j = 0; j < nx; ++j)
            {
                ...
                m_elems.push_back(new CFiniteElement2D<CRectangle, CRectangleBasis2y>{ nodes, temp, 6, 0 });
                //for (int ii = 0; ii < m_elems[m_elems.size() - 1]->GetDoFs(); ++ii)
                  //  cout << m_elems[m_elems.size() - 1]->GetNode(ii) << "\t";
                //cout << endl;
            }
...
const Mesh::Point CRegularMesh::GetNode(const unsigned int n) const
{
	return m_points[n];
}
...
const int CRegularMesh::interpolate(const int node) const
{
...
const int CRegularMesh::FindElement(const Point& test) const
{
	Point p[3];
	double volElem, mvol;
...
const CElement2D<>* CRegularMesh::GetElement(const unsigned int n) const
{
	if (n < m_elems.size())
		return m_elems[n];
	return nullptr;
...
const CElement<>* CRegularMesh::GetBoundary(const unsigned int n) const
{
	if (n < m_edges.size())
		return m_edges[n];
	return nullptr;
...
const double CRegularMesh::getSolution(const unsigned int element, const unsigned int node) const
{
	return 0.0;
}

const int CRegularMesh::updateSolution(const unsigned int element, const unsigned int node, const double value)
{
	return 0;
}
...
const int CRegularMesh::updateSolution(const std::vector<double>&)
{
	return 0;
}

const int CRegularMesh::updateSolution(const unsigned int element, const unsigned int node, CSolution * value)
{
	return 0;
}

const double CRegularMesh::getParameter(Parameters param, const unsigned int l, const Point & p) const
{
	if (l > m_elems.size())
		return 0.;
...
const double CRegularMesh::getParameter(Parameters param, const unsigned int l, const int i) const
{
	if (l > m_elems.size())
		return 0.;
...
const int CRegularMesh::setParameter(Parameters param, const double, const unsigned int)
{
	return 0;
}

const int CRegularMesh::setParameter(const CParameter& p, const unsigned int type)
{
	m_params[type] = p;
	return 0;
}

const int CRegularMesh::updateSolution(const unsigned int node, const double value)
{
	return 0;
}
